...
def get_twoway_sampler(mix, N=10):
    ...
    def sampler(x0, xf, t):
        with torch.no_grad():
            ...
            # timesteps = timesteps.to(x.device) # <-- CAUSAVA O ERRO

            # Modificado:
            # 1. Criamos o array com NumPy, garantindo que o tensor 'ts' seja movido para a CPU e convertido para NumPy.
            timesteps_np = np.linspace(mix.t0, ts.detach().cpu().numpy(), N)
            # 2. Convertemos o array NumPy para um tensor PyTorch e o movemos para o dispositivo (GPU/CPU) correto.
