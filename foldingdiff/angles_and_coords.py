...
import foldingdiff.nerf as nerf
...
def canonical_distances_and_dihedrals(
    ...
    warnings.filterwarnings("ignore", ".*elements were guessed from atom.*")
    ...
    for k, v in calc_angles.items():
        if not (float(np.nanmin(v)) >= -np.pi and float(np.nanmax(v)) <= np.pi):
...
def create_new_chain_nerf(
    ...
    if angles_to_set is None and dists_to_set is None:
        ...
        assert angles_to_set is not None, "Must specify angles_to_set if dists_to_set is given"
        assert dists_to_set is not None, "Must specify dists_to_set if angles_to_set is given"
    ...
    for a in angles_to_set:
        if a in required_dihedrals:
            continue
        assert a in dists_and_angles, f"Missing angle {a}"
        if a == "tau" or a == "N:CA:C":
    ...
    for d in dists_to_set:
        assert d in dists_and_angles.columns, f"Missing distance {d}"
        if d == "0C:1N":
