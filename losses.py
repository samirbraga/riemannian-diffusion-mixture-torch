    ...
    def loss_fn(modelf, modelb, x):
        ...
        if 'importance' in weight_type:
            ...
            ''' CORREÇÃO APLICADA AQUI 
             Original:
                A linha original (ou uma variação com np.random.uniform) poderia criar um array NumPy,
                o que causava o erro quando passado para a função sampler.
                t = torch.rand(shape[0], device=x.device) * (mix.tf - 2*eps) + eps

             Modificado:
                 Esta linha garante que 't' seja sempre um tensor PyTorch, resolvendo o bug de tipo.'''
