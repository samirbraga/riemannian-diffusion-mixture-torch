...
class Mixture(abc.ABC):
    ...
    def importance_cum_weight(self, t, eps):
        ...
        if self.beta_schedule._beta == 0:
            ...
            r = self.beta_schedule.beta_t(t) / self.beta_schedule.beta_t(self.t0+eps)
    ...
    def sample_importance_weighted_time(self, shape, eps, device, steps=100):
        Z = self.importance_cum_weight(self.tf-eps, eps=eps)
        quantile = torch.rand(shape, device=device) * Z
        lb = torch.ones_like(quantile) * (self.t0+eps)
        ub = torch.ones_like(quantile) * (self.tf-eps)
...
class DiffusionMixture(Mixture):
    ...
    def get_drift_fn(self, model, train=False):
        ...
        def drift_fn(manifold, x, t, seq_len):
            drift = model(x, t.unsqueeze(-1), manifold, seq_len)
            if self.pred:
